...
march_data <- data.frame(sick, deaths, recovered)
march_t <- transpose(march_data)
rownames(march_t) <- colnames(march_data)
colnames(march_t) <- top_march_c$Country.Region
barplot(as.matrix(march_t), main= "Top corona March", cex.names = 1, col = c("bisque1","firebrick1","darkorchid1"), xlab="Countries", ylab = "Corona Cases", las=2)
legend("topleft", c("Sick","Deaths", "Recovered"), fill = c("bisque1","firebrick1","darkorchid1"))
...
plot(italy_cases$Date, italy_cases$Diff, xlab = "Date", ylab = "Value", main= "Italy Corona", sub = "Daily deaths and new cases" ,pch = 8, col= "red", las=2)
points(italy_deaths$Date, italy_deaths$Diff, col= "blue", pch= 16)
legend("topleft", c("Cases", "Deaths"), col= c("red", "blue"), pch = c(8, 16))

#d
plot(italy_cases$Date, log(italy_cases$Diff), xlab = "Date", ylab = "Value", main= "Italy Corona", sub = "Daily deaths and new cases" ,pch = 8, col= "red", las=2)
points(italy_deaths$Date, log(italy_deaths$Diff), col= "blue", pch= 16)
legend("topleft", c("Cases", "Deaths"), col= c("red", "blue"), pch = c(8, 16))

#We can learn from the graph that the deaths and new cases increase at a similair rate, parallel to eachother.
...
corona <- full_join(deaths.agg, recovered.agg)
corona<- full_join(corona, cases.agg)

#b
names(corona)[1] <- "country"
cor_eco <- left_join(eco, corona)
...
cor_eco_march <- cor_eco_march[order(-cor_eco_march$nor_cases),][2:11,]
...
norm_cases <- cor_eco_march$nor_cases
norm_deaths <- cor_eco_march$nor_deaths
norm_recovered <- cor_eco_march$nor_recovered
...
colnames(nor_march_t) <- cor_eco_march$country

barplot(as.matrix(nor_march_t), main= "Top normalized corona March", cex.names = 1, col = c("bisque1","firebrick1","darkorchid1"), xlab="Countries", ylab = "Corona normalized Cases", las=2)
legend("topright", c("Sick","Deaths", "Recovered"), fill = c("bisque1","firebrick1","darkorchid1"))
...
plot(flt_march$pop65, flt_march$nor_cases, xlab = "% population over age 65", ylab = "Corona cases", main= "Corona cases and deaths vs % population over age 65", pch = 8, col= "black", las=2)
points(flt_march$pop65, flt_march$nor_deaths, col= "black", pch= 16)
legend("topleft", c("Cases", "Deaths"), col= c("black", "black"), pch = c(8, 16))
