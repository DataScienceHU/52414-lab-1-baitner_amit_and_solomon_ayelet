...
**method:We found the unique values of the date and checked the differences for each country every two days and with a loop we created a new vector that shows the changes.**
```{r}
#a
...
**In this gragh you can see the daily corona cases and deaths in italy from february to april. We used lines in the gragh to show the trend lines of corona cases and deaths. In the gragh you can see that the cases number and the deaths number are growing together and there is a tight connection between them in italy.**
...
**Q2.d.**
**In this gragh you can see daily corona cases and  deaths in italy from february to april. We used logarithem algoritem on the cases and deaths. We can learn from the graph that the deaths and new cases increase at a similair rate, parallel to eachother**
...
**In the process we received error notes that we gave the function dcast data frame instead of data table and we are aware of it and still prefer this format. Indeed, we made sure we didn't lose any information.**
...
**We arranged the data in descending order and by finding the data length and multiplying it by 0.9. We used this metric to cut the data, thus erasing ten percent of the countries with the least value.**
...
**During this difficult period we both have been following the Israeli news. We noticed that most if not all of the people leading this crisis in the country are men. In addition we saw many articals saying the countries that doing best corona responses are countries with women as leaders. As this raised some questions relating woman leadership We decided to research and compare the spreading of COVID19 in developed countries (included in the OECD) led by woman as oppose to countries led by men.  To answer our research question we create a data set including only countries led by woman . In order to get results as accurate as possible and filter out the effect of other parameters that might distort our results, we focus the research question in order to obtain framed results, we chose to delve deeper into OECD countries as many countries are led by women. 10 out of 36 countries in the OECD are currently led by woman (aproximatly 0.28 percent).**


**Research Question: Are OECD countries led by women countries really better at coping with the Corona epidemic?**

**Our assumptions:**
**- Better transportation of a country will result in a better deal with the Corona and this will be reflected in a low number of infections, a few deaths and a lot of exactly the citizens of the country.**
**- If women lead the epidemic better, we will see the countries leading women in low corona and death rates and high rates of testing, compared to men led by men.**

**In order to examine the reaserch question, from the Corona database we filtered out the countries that are in the OECD and also separated from them the countries led by women, and we added data on tests numbers in the OECD countries.**
...
**Both graphs (of normal coronary cases and of normal coronary deaths) show that women-led (pink-colored) countries are not in the lowest and highest places, so for men, the number of coroners and deaths is neither better nor worse. To conclude from this analysis that women do not necessarily lead countries better at the time of the epidemic, we note that the normalized numbers can be compared across countries despite a different population.**
