...
**Q1, we loaded the data from John's Hopkins Corona Database for cases, deaths and recovered number of people. Then we change the data class of the column "Date". To check the class we print the class of the column Date and you can see it is prints the class "date". **
```{r}
#a
...
**In this section, we have aggregated data using a built-in function and created a new data structure**
...
**Q2b, we created new columns for death and corona cases that show the day-to-day change in values so that the rate of change can also be checked. We used a loop that subtracts two consecutive days and creates a column of changes called Diff**
```{r}
#a
 ...
 **In this section we took the information we loaded from the Internet and changed it. We moved it from a long to wide format, creating new columns that include country and the unique values that were in the column feature and the columns value came from column Y2018. We used the dcast function we learn about in class. We also erase the first line because it was empty and the same for the first column.**
...
**In this section we took the eco data and arranged it according to GDP per capita. Then we change the data format to tibble and print the top fice countries with the highest GDP per capit. We thought it most relevant and focused to print the name of the country and the GDP per capita value**
...
**In this section we have created normalized columns (the information is divided into a thousand) for corona deaths, corona deaths, and recovery from it. We then filtered by date and arranged and found the first ten countries with the most cases normalized to population size**
...
**This graph shows the cases of corona and death in the all the countries (which the percentage of their population over 65 is at least 15) against the percentage of the population aged over 65. It can be noted that up to about 20 percent and some of the quantities actually increase with the respective population but then it ceases. The red shows the exceptional states where the number of dead is over 15% and the percentage of adults over 24%.
...
``` 
**בגרף זה הצגנו את כל מדינות ומספר מקרי הקורונה לכל מדינה. צבענו בצבע ורוד מדינות שמובלות
ע"י נשים ובתכלת מדינות עם מנהיגות גברית. ניתן לשים לב שמדינות עם הובלה נשיתאינן בטופ עשר מלבד אחת , בלגיה, אך גם לא נמצאות בתחתית. לפי גרף זה זה לא נכון שמדינות עם מנהיגות נשית בהכרח מתמודדות יותר טוב , להפך. מהנתונים משתקף שזה אינו המצב. באופן זהה ניתן לראות תוצאות אלה גם במקרי המוות. כמובן שכלל המספרים מנורמליםי על מנת להימנע מהשפעת גודל אוכלוסיה.**
**סיננו לסוף מרץ כתאריך שמייצג את שיא המגפה והוא מעט לפני תחילת ההקלות כך שייתן מספרים בשיאם**
...
New_Zealend <- oecd_cor_eco[which(oecd_cor_eco$country == "New Zealand"),]
...
Ireland <- oecd_cor_eco[which(oecd_cor_eco$country == "Ireland"),]
finland <- oecd_cor_eco[which(oecd_cor_eco$country == "Finland"),]

plot(Ireland$Date, Ireland$Diff.cases,type="l", lwd= 2, xlab = "Date", ylab = "Value", main= "Italy daily deaths and new cases (in log)",pch = 8, col= "red", las=1)
lines(finland$Date, finland$Diff.cases,type="l", lwd= 2, col= "blue", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("germany", "turkey"), col= c("red", "blue"), lty = c(1, 1))


plot(Ireland$Date, Ireland$nor_deaths,type="l", lwd= 2, xlab = "Date", ylab = "Value", main= "Italy daily deaths and new cases (in log)",pch = 8, col= "red", las=1)
lines(finland$Date, finland$nor_deaths,type="l", lwd= 2, col= "blue", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("germany", "turkey"), col= c("red", "blue"), lty = c(1, 1))

switz <- oecd_cor_eco[which(oecd_cor_eco$country == "Switzerland"),]
germany <- oecd_cor_eco[which(oecd_cor_eco$country == "Germany"),]
holland <- oecd_cor_eco[which(oecd_cor_eco$country == "Netherlands"),]
Spain <- oecd_cor_eco[which(oecd_cor_eco$country == "Spain"),]
UK <- oecd_cor_eco[which(oecd_cor_eco$country == "United Kingdom"),]
italy <- oecd_cor_eco[which(oecd_cor_eco$country == "Italy"),]
belgium <- oecd_cor_eco[which(oecd_cor_eco$country == "Belgium"),]
denmark <- oecd_cor_eco[which(oecd_cor_eco$country == "Denmark"),]
norway <- oecd_cor_eco[which(oecd_cor_eco$country == "Norway"),]
iceland <- oecd_cor_eco[which(oecd_cor_eco$country == "Iceland"),]
greece <- oecd_cor_eco[which(oecd_cor_eco$country == "Greece"),]
estonia <- oecd_cor_eco[which(oecd_cor_eco$country == "Estonia"),]


plot(Spain$Date, Spain$Diff.cases,type="l", lwd= 1, xlab = "Date", ylab = "Value", main= "OECD corona cases march 2020- men vs women",pch = 8, col= "green", las=1)
lines(germany$Date, germany$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("led by women", "led by man"), col= c("red", "green"), lty = c(1, 1))
lines(UK$Date, UK$Diff.cases,type="l", lwd=1, col= "green", pch= 16, ylim= c(0, 12))
lines(italy$Date, italy$Diff.cases,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
lines(belgium$Date, belgium$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(denmark$Date, denmark$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(norway$Date, norway$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(iceland$Date, iceland$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(greece$Date, greece$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(holland$Date, holland$Diff.cases,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
lines(estonia$Date, estonia$Diff.cases,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(switz$Date, switz$Diff.cases,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))

plot(Spain$Date, Spain$Diff.deaths,type="l", lwd= 1, xlab = "Date", ylab = "Value", main= "OECD deaths march 2020- men vs women",pch = 8, col= "green", las=1)
lines(germany$Date, germany$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("led by women", "led by man"), col= c("red", "green"), lty = c(1, 1))
lines(UK$Date, UK$Diff.deaths,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
lines(italy$Date, italy$Diff.deaths,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
lines(belgium$Date, belgium$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(denmark$Date, denmark$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(norway$Date, norway$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(iceland$Date, iceland$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(greece$Date, greece$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(holland$Date, holland$Diff.deaths,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
lines(estonia$Date, estonia$Diff.deaths,type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
lines(switz$Date, switz$Diff.deaths,type="l", lwd= 1, col= "green", pch= 16, ylim= c(0, 12))
