...
for (country in oecd_countries){
    country <- oecd_cor_eco[which(oecd_cor_eco$country == country),]
    
}

plot(1, type="l", lwd= 1, col= "red", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("led by women", "led by man"), col= c("red", "green"), lty = c(1, 1))
for (country in oecd_countries){
    country <- oecd_cor_eco[which(oecd_cor_eco$country == country),]
    lines(country$Date, country$Diff.nor_cases, type="l", lwd=1, col= "green", pch= 16, ylim= c(0, 12))
}

oecd_cor_eco_mat <- oecd_cor_eco[,c("country","Date", "Diff.nor_cases")]
mat <- as.matrix(oecd_cor_eco_mat[which(oecd_cor_eco_mat$country == "Spain"),])
mat_2 <- (oecd_cor_eco_mat[which(oecd_cor_eco_mat$country == "Germany"),])
matplot(mat, type="l", col= "red")
lines(mat_2, type="l", col= "red")
