...
**Solution:**  
**c- Showing that the two methods are the same by getting true in the checking**
...
barplot(as.matrix(march_t), main= "cases, deaths and recovered corona cases- March 2020", cex.names = 0.45, col = c("bisque1","firebrick1","darkorchid1"), xlab="Countries", ylab = "Corona Cases",  ylim= c(0, 200000))
legend("topright",title="Legend:", c("Sick","Deaths", "Recovered"), fill = c("bisque1","firebrick1","darkorchid1"))

```
**Q1.d In our opinion, the problem of presenting the information in such a way is that we lose information about whether there is any trend in the data and it is also difficult to notice the difference between countries in the different cases (death, patients and recoverers). They all displayed in stacked and not 
separately**
...
plot(italy_cases$Date, italy_cases$Diff,type="o", xlab = "Date", ylab = "Value", main= "Italy- daily corona deaths and new cases by date ",pch = 8, col= "red", las=1, ylim=c(0, 7000))
points(italy_deaths$Date, italy_deaths$Diff,type="o", col= "blue", pch= 16)
legend("topleft",title="Legend:", c("Cases", "Deaths"), col= c("red", "blue"), pch = c(8, 16))

#d
plot(italy_cases$Date, log(italy_cases$Diff),type="o", xlab = "Date", ylab = "Value", main= "Italy daily deaths and new cases (in log)",pch = 8, col= "red", las=1)
points(italy_deaths$Date, log(italy_deaths$Diff),type="o", col= "blue", pch= 16, ylim= c(0, 12))
legend("topleft",title="Legend:", c("Cases", "Deaths"), col= c("red", "blue"), pch = c(8, 16))
```
**Q2.d. We can learn from the graph that the deaths and new cases increase at a similair rate, parallel to eachother**
<br/><br/>
...
cases.agg <- cases.agg[which(cases.agg$Date == "2020-03-31"),]
...
corona <- corona[which(corona$Date == "2020-03-31"),]
cor_eco <- left_join(eco, corona, by="country")
...
#We notice that the top 10 countries in the orininal graph are mostly countries with large populations. The normalized graph allows us to get a better picture of corona cases in proportion to the countries population. That is why in this graph smaller countries such as Luxenburg (one of the smallest countries in the world) appear as well.# print
...
plot(flt_march$pop65, flt_march$nor_cases, xlab = "% population over age 65", ylab = "Corona cases", main= "Corona cases and deaths vs % population over age 65", pch = c(8), col=ifelse(flt_march$pop65 >24,'red','blue'), las=1)
points(flt_march$pop65, flt_march$nor_deaths, col= ifelse(flt_march$pop65 >24 | flt_march$nor_deaths>15,'red','blue'), pch= 16)
legend("topleft",title="Legend:", c("Cases", "Deaths", "Deaths >15", "pop65>24"), col= c("blue", "blue", "red", "red"), pch = c(8, 16, 8, 16))


```
**We notice that the top 10 countries in the orininal graph are mostly countries with large populations. The normalized graph allows us to get a better picture of corona cases in proportion to the countries population. That is why in this graph smaller countries such as Luxenburg (one of the smallest countries in the world) appear as well**
**We notice that there are countries spelled differently in the two data sets. We when throw the countries we thought were relevant in the assignment (US, Czehia and more).**
...
How Education Level in the countries affects State Citizens' Behavior in the Corona Plague and the state's success in dealing with the epidemic in the OECD countries**
